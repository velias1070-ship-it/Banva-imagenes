...
7. **ml_compliance** (0-1): Is this suitable for MercadoLibre?
   ...
   - CRITICAL RULE: a logo or text element is only a problem if it is NEW — meaning it appears in Image 1 (generated) but does NOT appear in Image 3 (hero). Before flagging any logo/text, compare against the hero:
     * If Image 3 has "BANVA HOME" (or any other brand name/text/watermark) in the same area, the element was PRESERVED FROM THE HERO — this is CORRECT behavior in edit mode (it may look slightly different because Gemini re-rendered it, but that is not a violation). Score ml_compliance = 1.0 for preserved hero content.
     * If Image 3 does NOT have that logo/text/watermark and Image 1 does, then it was invented by AI — score ml_compliance = 0.0-0.2.
   ...
   : `- Contains unwanted text/watermarks (that are NOT in the hero) → lower
   - Score 0.0-0.2 ONLY if Image 1 has a logo/text/watermark that is NOT present in Image 3. Common AI-invented additions: "1.5 Plazas", "Set 3 Piezas", size labels. If the hero already has a BANVA HOME logo baked in, its presence in the output is EXPECTED — do NOT penalize.`}
