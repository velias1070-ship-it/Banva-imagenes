...
async function regenerateJob(
  ...
  try {
    ...
    const originalSwatchBuffer = Buffer.from(swatchBuffer); // copy for Delta-E VLM fallback
...
You MUST RELOCATE these specific text elements so they no longer overlap the ${brand?.logo_position || 'top-left'} 250x250px corner. Move them horizontally to the right side of the image, OR shift them down below y=270. Keep their content unchanged. This is REQUIRED — do not skip this step.`;
    ...
    if (
      ...
      // bed_camera_angle column now stores stripe-visual-axis ('left_right' | 'top_bottom').
      let cachedAxis = (heroShot as Record<string, unknown>).bed_camera_angle as StripeVisualAxis | null | undefined;
      // Treat any legacy value (foot/side/top/other) as a stale cache and re-detect.
      if (cachedAxis !== 'left_right' && cachedAxis !== 'top_bottom') cachedAxis = null;
      if (!cachedAxis) {
        try {
          cachedAxis = await detectStripeVisualAxis(
            heroBuffer.toString('base64'),
            heroShot.mime_type || 'image/png'
          );
          if (cachedAxis && heroShot.id) {
            supabase.from('hero_shots').update({ bed_camera_angle: cachedAxis }).eq('id', heroShot.id).then(() => {});
          }
        } catch (err) {
          console.error('[regenerateJob] Stripe axis detection failed:', err);
        }
      }
      if (cachedAxis === 'top_bottom') {
        rotateSwatch = 90;
        logPipelineEvent(jobId, 'AUTO_ROTATE_SWATCH', '90deg', { reason: 'stripes_top_bottom' });
        console.log(`[regenerateJob] Auto-rotating swatch 90° (stripes appear top-bottom in camera)`);
      } else if (cachedAxis) {
        logPipelineEvent(jobId, 'STRIPE_AXIS_CHECKED', cachedAxis, { rotated: false });
      }
    }
...
Output: ${projectSettings.generation.resolution}px, RGB, PNG.`;
      ...
      if ((!detectedHeroOverlays || detectedHeroOverlays.length === 0) && heroShotId && heroBuffer.length > 0) {
        try {
          ...
          if (textAnalysis?.elements?.length) {
            ...
            // Mutate in-memory copy so the OVERLAY_RESTORED gate below
            // (which reads heroShot.text_elements) sees fresh data on this
            // same request, not only on subsequent regens.
            (heroShot as unknown as Record<string, unknown>).text_elements = detectedHeroOverlays;
    ...
    if (sizeNote) {
      prompt += sizeNote;
      console.log(`[regenerateJob] Size adjustment for SKU ${swatch.sku_suffix} (${effectiveShotType})`);
    }
    ...
    if (!isBrandOnly && !isMultiPass && !isInfografia && !skipDeltaE) {
      try {
        ...
        if (deltaEResult.swatchSource === 'vlm' && swatch.id) {
          ...
          Promise.resolve(supabase.from('swatches').update({ dominant_color_hex: hex }).eq('id', swatch.id))
            .catch((err: unknown) => console.error('[regenerateJob] Failed to cache VLM swatch hex:', err));
        }
      } catch (driftErr) {
    ...
    // If this regen is not brand-only, the new output invalidates the
    // existing pre-brand backup. Leaving that backup on disk causes the
    // next Brand click to load the stale version and overwrite the new
    // swatch result with an old fabric (repro: job 64c4c9d1).
    if (!isBrandOnly) {
      const preBrandPath = `projects/${projectId}/generated/${jobId}_pre_brand.png`;
      try {
        await supabase.storage.from('images').remove([preBrandPath]);
        logPipelineEvent(jobId, 'PRE_BRAND_INVALIDATED', 'nuevo output reemplaza backup');
      } catch {
        // No-op: remove on a non-existent file is not an error for Supabase,
        // but swallow just in case the storage driver surfaces one.
      }
    }
